...
Our second model will be using a decision tree to evaluate which variables are strongly associated with Alzheimer’s disease. By utilizing a decision tree, we can easily interpret data and be able to deal with non-linear relationships. Additionally, decision trees have proven to handle categorical data effectively. However, this model can lead to high variance, which can be managed with appropriate techniques.
...
```
\ 
For this base tree we had a 93.95% accuracy rate and an error rate of 6.04%, demonstrating the unpruned trees ability to accurately predict whether or not a person is diagnosed with Alzheimer's Disease. 
